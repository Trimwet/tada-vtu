...
export default function BuyDataPage() {
    ...
    const handlePurchase = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (inlomaxData.status !== "success" || (inlomaxData.balance ?? 0) < selectedPlanDetails.price) {
                toast.warning("Service temporarily unavailable", "Data stock is currently low. Please try again later or contact support.");
                return;
            }
        } catch {
    ...
    return (
        <div className="min-h-screen bg-background">
                ...
                <Card className="border-border">
                    ...
                    <CardContent>
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="space-y-1 flex-1">
                                    <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${purchaseMode === 'buy' ? 'bg-green-500 animate-pulse' : 'bg-blue-500'}`} />
                                        {purchaseMode === 'buy' ? 'Direct Purchase' : 'Data Vault Mode'}
                                    </h3>
                                    <p className="text-[11px] text-muted-foreground leading-tight">
                                        {purchaseMode === 'buy'
                                            ? 'The bundle will be delivered to the recipient instantly.'
                                            : 'Payment is deducted now. Data sits in your vault for 1-tap delivery later.'}
                                    </p>
                                </div>
                                ...
                                <div className="flex flex-col items-center gap-1.5 shrink-0">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setPurchaseMode(purchaseMode === 'buy' ? 'park' : 'buy')}
                                        className={`relative inline-flex h-6 w-12 items-center rounded-full border-2 transition-all duration-300 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${purchaseMode === 'park'
                                                ? 'bg-green-500 border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.3)]'
                                                : 'bg-white/10 border-white/10 hover:border-white/30'
                                            }`}
                                    >
                                        <span
                                            className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-300 ${purchaseMode === 'park'
                                                    ? 'translate-x-6'
                                                    : 'translate-x-1'
                                                }`}
                                        />
                        ...
                        <form onSubmit={handlePurchase} className="space-y-6">
                            ...
                            <Button
                                type="submit"
                                className={`w-full font-semibold h-12 transition-smooth ${purchaseMode === 'park'
                            ...
                            >
                                {(isProcessing || isParking) ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                        <IonIcon
                                            name={purchaseMode === 'park' ? 'wallet-outline' : 'send-outline'}
                                            size="20px"
                                        />
                                        {purchaseMode === 'park' ? 'Save to Vault' : 'Send Data Now'}
                                    </div>
                                )}
                            </Button>
            ...
            <VerifyPinModal
                ...
                title={purchaseMode === 'park' ? "Authorize Data Parking" : "Authorize Purchase"}
                description={`Enter PIN to ${purchaseMode === 'park' ? 'park' : 'buy'} ${selectedPlanDetails?.size || ""} ${selectedNetwork} data`}
            />
        </div>
    );
}

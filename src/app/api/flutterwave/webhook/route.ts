  ...
  try {
    ...
    if (event === 'charge.completed' && data.status === 'successful') {
      ...
      if (!userId && data.payment_type === 'bank_transfer') {
        ...
        // Fallback 1: Lookup by email if available
      ...
      if (!userId) {
        ...
        await supabase.from('pending_payments').upsert(
          { flutterwave_ref: flwRef, tx_ref: txRef, amount, status: 'pending' },
      ...
      // Calculate credit using tiered fee structure
      ...
      // processDeposit now calls Core — no Supabase client needed
      await processDeposit({
        ...
        description: isBankTransfer
          ? `Bank transfer (₦${fee} fee deducted)`

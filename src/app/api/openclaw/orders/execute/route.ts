  ...
  try {
    ...
    if (!orderId || !userId) {
      return NextResponse.json(
        openclawError('Missing required fields: orderId, userId', 'MISSING_FIELDS'),
        { status: 400 }
      );
    }
    ...
    // ── Fetch the pre-created order record ────────────────────────────────────
    type OrderData = {
      id: string;
      user_id: string;
      type: string;
    ...
    if (orderError || !order) {
      return NextResponse.json(openclawError('Order not found', 'ORDER_NOT_FOUND'), { status: 404 });
    }
    if (order.status !== 'pending') {
      return NextResponse.json(
        openclawError(`Order already ${order.status}. Cannot execute.`, 'ORDER_ALREADY_PROCESSED'),
        { status: 400 }
      );
    }
    if (order.type !== 'data') {
      return NextResponse.json(
        openclawError('Invalid order type. Only data orders are supported.', 'INVALID_ORDER_TYPE'),
        { status: 400 }
      );
    }
    if (!order.phone_number || !order.service_id || !order.network) {
      return NextResponse.json(
        openclawError('Order is missing required information', 'INVALID_ORDER_DATA'),
        { status: 400 }
      );
    }

    // Verify account is active (fast Supabase read — no balance read needed, Core handles that)
    ...
    if (profileError || !profile) {
      return NextResponse.json(openclawError('User not found', 'USER_NOT_FOUND'), { status: 404 });
    }
    if (!profile.is_active) {
      return NextResponse.json(
        openclawError('Account is inactive. Please contact support.', 'ACCOUNT_INACTIVE'),
        { status: 403 }
      );
    }

    const purchaseAmount = Math.abs(order.amount);
    // Use EXEC_{orderId} as the Core reference so it doesn't collide with the
    // pre-existing order record reference in the transactions table.
    const coreRef    = `EXEC_${order.reference}`;
    const description = order.description || `OpenClaw data purchase ${order.reference}`;
    ...
    try {
      ...
      // Mark the order as failed so the bot can surface a clean error
      await supabase
        .from('transactions')
      ...
      console.error('[OPENCLAW EXECUTE] Core debit failed:', debitError);
      return NextResponse.json(
        openclawError('Payment processing failed. Please try again.', 'DEBIT_FAILED'),
        { status: 500 }
      );
    }
    ...
    try {
      console.log(
        `[OPENCLAW EXECUTE] Calling Inlomax: ${order.network} to ${order.phone_number}, serviceID: ${order.service_id}`
      );
      const result = await purchaseDataInlomax({
        serviceID: order.service_id,
        phone: order.phone_number,
      });
      console.log(`[OPENCLAW EXECUTE] Inlomax response:`, result.status, result.message);

      if (result.status === 'success') {
        // Update the original order record (used by bot for status polling)
        await supabase
          .from('transactions')
          .update({ status: 'success', external_reference: result.data?.reference })
          .eq('id', orderId);

        // Also mark Core's debit record as success
        await supabase
          .from('transactions')
          .update({ status: 'success', external_reference: result.data?.reference })
      ...
      }).catch((e) => console.error('[OPENCLAW EXECUTE] Refund failed:', e));

      await supabase.from('transactions').update({ status: 'failed' }).eq('id', orderId);

      return NextResponse.json(
        openclawError(result.message || 'Data purchase failed. Please try again.', 'PURCHASE_FAILED'),
        { status: 400 }
      );

    } catch (apiError) {
      console.error('[OPENCLAW EXECUTE] Provider error:', apiError);

      // Provider threw — refund the debit
      await coreRefund({
        userId,
        amount: purchaseAmount,
        reference: `REFUND_${coreRef}`,
      ...
      if (apiError instanceof ServiceUnavailableError) {
        return NextResponse.json(
          openclawError('Service is unavailable. Please try again later.', 'SERVICE_UNAVAILABLE'),
          { status: 503 }
        );
      }

      const errorMessage = apiError instanceof Error ? apiError.message : 'Service temporarily unavailable';
      return NextResponse.json(openclawError(errorMessage, 'PURCHASE_FAILED'), { status: 500 });
    }
  } catch (error) {
    console.error('[OPENCLAW EXECUTE] Unexpected error:', error);
    return NextResponse.json(
